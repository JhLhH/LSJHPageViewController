//
//  LLPageTableViewController.m
//  CocoaLumberjack
//
//  Created by 李俊恒 on 2018/11/23.
//

#import "LLPageTableViewController.h"
#import<LSJHCategory/LLCategory.h>

NSString * const LLTableControllerDidAddToSuperViewNotification =
@"LLTableControllerDidAddToSuperViewNotification";
NSString * const LLTableControllerDidFullyDisplayedNotification =
@"LLTableControllerDidFullyDisplayedNotification";
NSString * const LLTabControllerChildControllerChangeContentOffstNotification =
@"LLTabControllerChildControllerChangeContentOffstNotification";
static NSInteger const kWMUndefinedIndex           = -1;
static NSInteger const kWMControllerCountUndefined = -1;

@interface LLPageTableViewController () <UITableViewDelegate, UITableViewDataSource> {
    CGFloat _targetX;
...
@implementation LLPageTableViewController
    ...
    if (_acceptNotification) {
        ...
        addObserver:self
           selector:@selector(changeOffset:)
               name:LLTabControllerChildControllerChangeContentOffstNotification
             object:nil];
    }
}
...
- (void)setCachePolicy:(LLPageTableControllerCachePolicy)cachePolicy
{
    _cachePolicy = cachePolicy;
    if (cachePolicy != LLPageTableControllerCachePolicyDisabled) {
...
- (void)didEnterController:(UIViewController *)vc atIndex:(NSInteger)index
{
    ...
    if (self.preloadPolicy == LLPageTableControllerPreloadPolicyNear) return;
...
- (void)postAddToSuperViewNotificationWithIndex:(int)index
{
    ...
    [[NSNotificationCenter defaultCenter]
    postNotificationName:LLTableControllerDidAddToSuperViewNotification
                  object:self
                userInfo:info];
}
/**
...
- (void)postFullDisplayNotificationWithCurrentIndex:(int)index
{
    ...
    NSDictionary * info = @{ @"index" : @(index),
                             @"title" : [self titleAtIndex:index] };
    [[NSNotificationCenter defaultCenter]
    postNotificationName:LLTableControllerDidFullyDisplayedNotification
                  object:self
                userInfo:info];
}
...
- (void)setup
{
    ...
    self.preloadPolicy                        = LLPageTableControllerPreloadPolicyNever;
    self.cachePolicy                          = LLPageTableControllerCachePolicyNoLimit;
...
- (void)removeViewController:(UIViewController *)viewController atIndex:(NSInteger)index
{
    ...
    // 放入缓存
    if (self.cachePolicy == LLPageTableControllerCachePolicyDisabled) {
        return;
    }
...
{
    self.cachePolicy = LLPageTableControllerCachePolicyBalanced;
...
{
    self.cachePolicy = LLPageTableControllerCachePolicyHigh;
}
...
- (void)viewDidLoad
{
    ...
    initWithFrame:CGRectMake(0, LLTopHeight, ScreenWidth, ScreenHeight - LLTopHeight)
...
- (void)didReceiveMemoryWarning
{
    ...
    self.cachePolicy = LLPageTableControllerCachePolicyLowMemory;
    // 取消正在增长的 cache 操作
    [NSObject cancelPreviousPerformRequestsWithTarget:self
                                             selector:@selector(growCachePolicyAfterMemoryWarning)
                                               object:nil];
